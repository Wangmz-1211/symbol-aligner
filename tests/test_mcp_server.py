...
def test_query_candidates_tool():
    out = json.loads(mcp_server.query_candidates("tmpVal", "mappings/example.json"))
    assert out["identifier"] == "tmpVal"
    assert out["top_k"][0]["legacy"] == "tmpVal"
...
def test_preview_tool_does_not_write(tmp_path):
    ...
    assert any(c["new"] == "temporaryValue" for c in out["changes"])


def test_align_batch_dry_run(tmp_path):
    (tmp_path / "a.py").write_text("x = tmpVal\n")
    (tmp_path / "b.py").write_text("y = calcTtl\n")
    (tmp_path / "c.txt").write_text("z = tmpVal\n")  # ignored: not .py
    out = json.loads(mcp_server.align_batch(str(tmp_path), "mappings/example.json"))
    assert out["file_count"] == 2
    assert (tmp_path / "a.py").read_text() == "x = tmpVal\n"  # dry-run, untouched
